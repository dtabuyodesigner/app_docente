...
def seleccionar_encargado():
    try:
        ...
        # 8. Guardar (Usamos DELETE + INSERT para máxima compatibilidad con versiones antiguas de SQLite)
        cur.execute("DELETE FROM encargados WHERE fecha = ?", (fecha,))
        cur.execute("""
        ...
        print(f"Error en seleccionar_encargado: {str(e)}")
        traceback.print_exc()
